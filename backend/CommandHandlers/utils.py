...
def validate_new_name(env: Environment, name: str, allowed_category: Optional[str] = None) -> bool:
    from backend.Parser import LOGICAL_CONNECTIVES, QUANTIFIERS
...
def enforce_no_shadowing(env: Environment, target_name: str, out_name: str, category: str = "formula") -> bool:
    """Enforces that inherited objects cannot be shadowed (modified in-place) and validates new names."""
    if out_name == target_name:
        local_registry = env.local_formulae if category == "formula" else env.local_terms
        registry = env.formulae if category == "formula" else env.terms
        if target_name not in local_registry and target_name in registry:
            print(f"Error: '{target_name}' is inherited from a parent environment and cannot be modified in-place. You must provide a fresh <out> name.")
            return False
        return True
    else:
        return validate_new_name(env, out_name, category)
...
def handle_variable_capture_interactive(env: Environment, e: Exception, f_clone, symbol: str, command_queue: list = None, inputs_collected: list = None):
    ...
    for cap_var in e.capturing_vars:
        while True:
            ...
            from backend.SubstitutionManager import substitute_bound
