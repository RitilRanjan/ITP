...
def snapshot_env_state(env) -> dict:
    return {
        "id": id(env),
        "goal_formula_name": env.goal_formula_name,
        "variables": dict(env.local_variables),
        "dummy_variables": dict(env.local_dummy_variables),
        "meta_variables": dict(env.local_meta_variables),
        "propositional_variables": dict(env.local_propositional_variables),
        "terms": dict(env.local_terms),
        "formulae": dict(env.local_formulae),
        "theorems": set(env.local_theorems),
        "long_terms": dict(env.local_long_terms),
        "long_formulae": dict(env.local_long_formulae)
    }

def compute_env_delta(before: dict, after: dict) -> dict:
    delta = {"added": {}, "removed": {}, "modified": {}}
    if before.get("goal_formula_name") != after.get("goal_formula_name"):
        delta["goal_formula_name_changed"] = {
            "before": before.get("goal_formula_name"),
            "after": after.get("goal_formula_name")
        }
        
    for k in before.keys():
        if k in ("id", "goal_formula_name"):
            continue
            
        if k == "theorems":
            added = after[k] - before[k]
            removed = before[k] - after[k]
            if added: delta["added"][k] = added
            if removed: delta["removed"][k] = removed
            continue
            
        # For dictionaries (everything except theorems)
        before_keys = set(before[k].keys())
        after_keys = set(after[k].keys())
        
        added_keys = after_keys - before_keys
        removed_keys = before_keys - after_keys
        common_keys = before_keys.intersection(after_keys)
        
        added = {name: after[k][name] for name in added_keys}
        removed = {name: before[k][name] for name in removed_keys}
        
        modified = {}
        for name in common_keys:
            if before[k][name] is not after[k][name]:
                # Store the before and after states for undo/redo
                modified[name] = {"before": before[k][name], "after": after[k][name]}
                
        if added: delta["added"][k] = added
        if removed: delta["removed"][k] = removed
        if modified: delta["modified"][k] = modified
...
class RecycleBinManager:
    ...
    def record_command(self, line_str: str, before_snapshot: dict, before_env: Any, after_env: Any, mission_entered: bool, mission_resolved: bool):
        delta = compute_env_delta(before_snapshot, snapshot_env_state(after_env))
        if mission_entered:
    ...
    def undo(self, env: Any) -> Tuple[bool, Any, str]:
        ...
        # 4. Revert modified objects
        if "modified" in delta:
            for k, mods in delta["modified"].items():
                dict_ref = getattr(env, f"local_{k}")
                for name, states in mods.items():
                    if name in dict_ref:
                        dict_ref[name] = states["before"]
                        
        # 5. Restore goal formula name
        if "goal_formula_name_changed" in delta:
            env.goal_formula_name = delta["goal_formula_name_changed"]["before"]
    ...
    def redo(self, env: Any) -> Tuple[bool, Any, str]:
        ...
        # 4. Re-apply modified objects
        if "modified" in delta:
            for k, mods in delta["modified"].items():
                dict_ref = getattr(env, f"local_{k}")
                for name, states in mods.items():
                    if name in dict_ref:
                        dict_ref[name] = states["after"]
            
        # 5. Restore goal formula name
        if "goal_formula_name_changed" in delta:
            env.goal_formula_name = delta["goal_formula_name_changed"]["after"]
