...
from backend.AST import Variable, PropositionalVariable, DummyVariable, Function, Relation
...
class AutocompleteEngine:
    ...
    def get_suggestions(self, partial_command: str, env: Environment, game_allowed_commands: List[str] = None) -> List[str]:
        ...
        elif cmd_name in ["left", "right", "and", "iff", "imply", "intro", "neg-", "neg+", "dt"]:
            # target is arg 1 or implied
            options = formula_names
            return [opt for opt in options if opt.startswith(current_token)]
            
        elif cmd_name in ["cf", "ct", "cp", "mission", "auto", "search", "backward_search", "advanced_search"]:
