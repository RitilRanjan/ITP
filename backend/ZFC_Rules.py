from backend.AST import (
    Node, FormulaNode, Variable, LongTerm, LongFormula, Connective, Quantifier, DummyVariable, PropositionalVariable, MetaVariable, is_structurally_equal,
    Function, FunctionType, Relation, RelationType
)
...
def axiom_replacement(formula: FormulaNode) -> bool:
    ...
    if not isinstance(left, LongFormula) or left.definition_name != "∈" or "t1" not in left.term_placeholders:
        return False
    if left.term_placeholders["t1"].name != v or left.term_placeholders["t2"].name != B:
        return False
    ...
    if not isinstance(c1, LongFormula) or c1.definition_name != "∈" or "t1" not in c1.term_placeholders:
        return False
    if c1.term_placeholders["t1"].name != u:
        return False
    A = c1.term_placeholders["t2"].name
    ...
    if not isinstance(ant_right, LongFormula) or ant_right.definition_name != "=" or "t1" not in ant_right.term_placeholders:
        return False
    arn1, arn2 = ant_right.term_placeholders["t1"].name, ant_right.term_placeholders["t2"].name
    ...
    p_uv, p_uw = ant_left.term_placeholders["t1"], ant_left.term_placeholders["t2"]
