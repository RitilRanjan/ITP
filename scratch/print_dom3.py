...
async def click_and_run(page, target_sel, cmd_text, input_vals=None, wait_time=2000):
    ...
    for attempt in range(10):  # Retry up to 10 times (10 seconds)
        ...
        try:
            ...
            if "Timeout" not in str(type(e)):
                print(f"Exception trying to click in main page: {e}")
            pass
        ...
        for i in range(5):
            try:
                ...
                if "Timeout" not in str(type(e)):
                    print(f"Exception trying to click in frame {i}: {e}")
                pass
        if clicked:
            break
...
async def main():
    async with async_playwright() as p:
        ...
        with open('scratch/dom3.html', 'w') as f:
            f.write(content)
            
        print("DOM saved to dom3.html")
        await browser.close()
