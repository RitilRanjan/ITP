...
async def main():
    async with async_playwright() as p:
        ...
        try:
            ...
            # Find the input box. It's likely inside an iframe from st_keyup
            # Let's just find the iframe with 'streamlit_keyup' in the src
            frame = page.frame_locator('iframe[title="streamlit_keyup.st_keyup"]')
            
            input_el = frame.locator('input').first
            await input_el.fill("cv x")
            # For st_keyup we need to press Enter
