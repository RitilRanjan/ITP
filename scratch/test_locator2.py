...
async def main():
    async with async_playwright() as p:
        ...
        count1 = await page.locator('span[data-target="russel"]').count()
        print(f"Main page span[data-target='russel'] count: {count1}")
        count2 = await page.locator('.interactive-name').count()
        print(f"Main page .interactive-name count: {count2}")
