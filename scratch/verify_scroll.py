...
def test_scroll():
    with sync_playwright() as p:
        ...
        # Force click the sidebar toggle
        try:
            page.locator('[data-testid="stSidebarCollapsedControl"]').click(force=True, timeout=2000)
            time.sleep(1)
        except Exception as e:
            print("Toggle not clicked:", e)
            try:
                page.locator('button[kind="header"]').first.click(force=True, timeout=2000)
                time.sleep(1)
            except Exception as e2:
                print("Fallback toggle not clicked:", e2)
        
        # Click on Games tab
        page.locator("text=🎮 Games").click(force=True)
        time.sleep(2)
        
        # Click on natural number game
        page.locator("text=natural number game").click(force=True)
        time.sleep(2)
        
        # Click level 3
        page.locator("text=Level 3").click(force=True)
        time.sleep(2)
