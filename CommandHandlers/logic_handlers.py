...
from AST import Variable, Quantifier, Connective, Node
...
def handle_left_right(env: Environment, args_str: str, cmd: str) -> None:
    ...
    if len(cmd_args) == 0:
        if env.goal_formula_name is None:
            ...
            return
        if env.goal_formula_name not in env.local_formulae:
            print(f"Error: In-place goal modification requires the goal '{env.goal_formula_name}' to be created in the current environment.")
            return
...
def handle_and(env: Environment, args_str: str) -> Environment:
    ...
    if out_left_name in env.formulae and out_left_name != target_name:
        ...
        if out_left_name != target_name and not validate_new_name(env, out_left_name, "formula"):
            return env
    ...
    if out_right_name in env.formulae and out_right_name != target_name:
        ...
        if out_right_name != target_name and not validate_new_name(env, out_right_name, "formula"):
            return env
    ...
    if is_goal_target:
        ...
        if out_left_name == target_name:
            if target_name not in env.local_formulae and target_name in env.formulae:
                env.local_formulae[out_left_name] = clone_ast(psi_node)
            else:
...
def handle_imply(env: Environment, args_str: str) -> None:
    ...
    if out_consequent_name in env.formulae and out_consequent_name != target_name:
        ...
        if out_consequent_name != target_name and not validate_new_name(env, out_consequent_name, "formula"):
            return
    ...
    if out_antecedent_name in env.formulae and out_antecedent_name != target_name:
        ...
        if out_antecedent_name != target_name and not validate_new_name(env, out_antecedent_name, "formula"):
            return
        f2_action = 'new'
    ...
    if is_goal_target:
        ...
        if out_consequent_name == target_name:
            if target_name not in env.local_formulae and target_name in env.formulae:
                env.local_formulae[out_consequent_name] = clone_ast(phi_node)
            else:
        ...
        if f2_action == 'new':
            if out_antecedent_name == target_name:
                if target_name not in env.local_formulae and target_name in env.formulae:
                    env.local_formulae[out_antecedent_name] = clone_ast(psi_node)
                else:
...
def handle_intro(env: Environment, args_str: str) -> None:
    ...
    if out_name in env.formulae and out_name != target_name:
        ...
        if out_name != target_name and not validate_new_name(env, out_name, "formula"):
            return
            
    # Assign the new formula to out_name
    if out_name == target_name:
        if target_name not in env.local_formulae and target_name in env.formulae:
            env.local_formulae[out_name] = new_body_node
        else:
